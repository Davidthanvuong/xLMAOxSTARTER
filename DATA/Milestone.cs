...
public class Milestone
{
    ...
    public static List<Milestone> Milestones = new()
    {
        new Milestone(
            0, "XP Wizard",
            () => $"Have {AppData.Milestones[0].ScoreFunc().FloorMult(1000)} XP in total",
            () => AppData.XPdata.totalXP / 1000
            ),
        new Milestone(
            1, "Healthy Consumer",
            () => $"Have {AppData.Milestones[1].ScoreFunc().FloorMult(200)} XP gained from good apps in total",
            () => AppData.XPdata.healthyTotalXP / 200
            ),
        new Milestone(
            2, "Successful Achiever",
            () => $"Have {AppData.Milestones[2].ScoreFunc().FloorMult(200)} XP gained from milestones in total",
            () => AppData.XPdata.milestoneTotalXP / 200
            ),
        new Milestone(
            3, "Energetic Wealth",
            () => $"Have {AppData.Milestones[3].ScoreFunc().FloorMult(500)} acquired XP at the same time",
            () => AppData.XPdata.highestXP / 500
            ),
        new Milestone(
            4, "Productivity Paragon",
            () => $"Avoid using bad apps for consecutive {AppData.Milestones[4].ScoreFunc().FloorMult(4, 5)} hours",
            () => Math.Min(0, (AppData.MiscGame.highestNoBadAppTime / 3600) * 4 - 5)
            ),
        new Milestone(
            5, "Tempo Master",
            () => $"Be offline for consecutive {AppData.Milestones[5].ScoreFunc().FloorMult(5, 4)} hours",
